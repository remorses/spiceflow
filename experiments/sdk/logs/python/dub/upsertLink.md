...
- Create a LinkUpsertRequest type for the request body
- Use the existing LinkSchema for the response type
- Define LinkGeoTargeting type for the geo targeting object

2. The method needs to:
- Accept all possible request parameters
- Handle optional fields using Optional types
- Use aiohttp for the PUT request
- Handle error responses
- Return the LinkSchema response

3. We'll add the method to the ExampleClientAsync class since it already has the base infrastructure for making requests.
...
class LinkGeoTargeting(TypedDict, total=False):
    AF: str
    AL: str
    DZ: str
...
class LinkUpsertRequest(TypedDict, total=False):
    url: str
    domain: Optional[str]
    key: Optional[str]
...
class ExampleClientAsync:
    ...
    # PUT /links/upsert - Upsert a link
    async def upsert_link(self, request: LinkUpsertRequest) -> Dict[str, Any]:
        """
        Upsert a link for the authenticated workspace by its URL.
        If a link with the same URL already exists, return it (or update it if there are any changes).
        Otherwise, a new link will be created.
        
        Route: PUT /links/upsert
        Method: PUT
        Tags: Links
        """
