...
## Edge Cache (`cloudflareCache` middleware)

Workers run **before** the CDN cache, so responses generated by a Worker are never cached at the edge automatically — even if you set `Cache-Control: s-maxage=600`. The `cloudflareCache` middleware bridges this gap using the Cache API (`caches.default`), letting you control edge caching with standard headers.
...
cloudflareCache({
  ...
  // Vary headers for per-variant caching
...
| | Edge Cache (`cloudflareCache`) | KV Cache (above) |
|---|---|---|
