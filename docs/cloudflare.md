...
## Split Sub-Apps (Dynamic Workers)

Cloudflare Workers have a **bundle size limit** (1 MB free, 10 MB paid). Large apps with many routes can hit this limit or suffer from slow cold starts because the entire Worker code must be loaded before any request is handled. `.split()` solves both problems by breaking your app into smaller, independently-loaded sub-apps that run as **Dynamic Workers**.

```
┌──────────────────────────────────────────────────────────────────────────────────────────┐
│  Parent Worker (main.tsx)                                                                │
│                                                                                          │
│  GET /             ──► root handler (runs in parent)                                     │
│  GET /health       ──► health handler (runs in parent)                                   │
│                                                                                          │
│  /admin/*   ──► LOADER.get() ──► Dynamic Worker (admin.tsx, isolated V8 isolate)         │
│  /billing/* ──► LOADER.get() ──► Dynamic Worker (billing.tsx, isolated V8 isolate)       │
└──────────────────────────────────────────────────────────────────────────────────────────┘
```

Each split sub-app is bundled into its own set of chunks, uploaded as static assets, and loaded on demand via the `LOADER` binding. The parent Worker stays small and fast. Sub-apps cold-start independently, so a request to `/admin/users` never loads the billing code.

### Setup

Add `worker_loaders` and `assets` bindings to your wrangler config:
...
Register split sub-apps with `.split()`:
...
Each sub-app is a standalone Spiceflow instance:
...
The Vite plugin detects `.split()` calls at build time and treats each dynamic import as a separate entry point. During `generateBundle`:

1. Each split entry's chunk and its transitive dependencies are collected
2. A content hash is computed from all module sources
3. A thin wrapper module is generated that re-exports the sub-app's `fetch()`
4. All split-only modules are removed from the main Worker bundle
5. The import call in the parent is rewritten to return manifest data (entry, modules, content hash) instead of the actual module

At runtime, the parent Worker calls `LOADER.get(contentHash, factory)` where the factory fetches the sub-app's chunks from `ASSETS`. Cloudflare caches the compiled isolate by content hash, so subsequent requests skip the code loading step.

### Middleware

Parent middleware runs **before** split sub-app dispatch. This means authentication, logging, CORS, and other cross-cutting concerns work without duplicating them in every sub-app:

```ts
export const app = new Spiceflow()
  .use((ctx, next) => {
    ctx.response.headers.set('x-app', 'my-worker')
    return next()
  })
  .split('/admin/*', () => import('./admin'))
```

Middleware can also short-circuit before the sub-app is loaded, saving the cold start cost entirely:
...
On non-Cloudflare runtimes, `.split()` works identically to a regular dynamic import. The sub-app module is loaded via `import()` and executed in the same process. No `LOADER` or `ASSETS` bindings are needed. This means the same code runs everywhere without conditional logic.

### Limitations

Split sub-apps on Cloudflare run in **isolated V8 isolates**. This provides security and performance isolation, but comes with trade-offs:
