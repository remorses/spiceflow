...
- CreateLinkRequest: Represents all possible fields for creating a link
- LinkSchema: Represents the response structure for a created link
- LinkGeoTargeting: Represents the geo targeting structure

2. The create_link method needs to:
- Accept all possible link creation parameters
- Make an async POST request to /links
- Handle the response and error cases
- Return the created link data

3. We'll use the existing ExampleClientAsync class and its fetch method for making requests.
...
class LinkGeoTargeting(BaseModel):
    ...
    # ... (add all country codes from schema)
...
class CreateLinkRequest(BaseModel):
    ...
    trackConversion: Optional[bool] = False
    archived: Optional[bool] = False
    publicStats: Optional[bool] = False
    tagId: Optional[str] = None
    tagIds: Optional[Union[str, List[str]]] = None
    tagNames: Optional[Union[str, List[str]]] = None
    ...
    geo: Optional[LinkGeoTargeting] = None
    doIndex: Optional[bool] = False
    ...
    webhookIds: Optional[List[str]] = None

class LinkSchema(CreateLinkRequest):
    id: str
    shortLink: str
    qrCode: str
    ...
    workspaceId: str
    clicks: int = 0
    lastClicked: Optional[datetime] = None
    leads: int = 0
    sales: int = 0
    saleAmount: int = 0
    createdAt: datetime
...
class ExampleClientAsync:
    ...
    async def create_link(self, link_data: CreateLinkRequest) -> LinkSchema:
        """
        ...
        response = await self.fetch(
            method="POST",
            path="/links",
            body=link_data.dict(exclude_none=True)
        )
        
        if response.status == 200:
            return LinkSchema(**await response.json())
        else:
