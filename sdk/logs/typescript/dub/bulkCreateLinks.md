...
- Create a type for the link creation request body based on the schema
- Use the existing LinkSchema type for the response
- Define error types based on the possible error responses

2. The method implementation needs to:
- Accept an array of link creation objects
- Use the existing fetch method from ExampleClient
- Handle error responses properly
- Return the created links array

3. We'll add the method to the ExampleClient class with proper typing and documentation.
...
// Add LinkSchema type if not already defined
type LinkSchema = {
  id: string;
  domain: string;
  key: string;
