...
1. **`SpiceflowRegister` type registry** — add a single `declare module` at the bottom of your app entry and all typed APIs (`router`, `useLoaderData`, `useRouterState`, `createSpiceflowFetch`) work without generics. No more `<typeof app>` everywhere:
...
2. **Built-in OpenTelemetry tracing** — pass a `tracer` to the Spiceflow constructor and every request automatically gets spans for middleware, handlers, loaders, layouts, pages, and RSC serialization. No monkey-patching or plugins required. The tracer interface is structurally compatible with `@opentelemetry/api` so there's zero runtime dependency:
   ...
   Each span includes standard HTTP attributes following OTel semantic conventions. Errors are recorded with `recordException`. When no tracer is passed, all instrumentation points are skipped with zero overhead.

   Add `serverTiming: true` alongside `tracer` to expose request spans as a `Server-Timing` response header visible in Chrome DevTools. All route types also receive `span` and `tracer` on their context object for custom attributes, caught exception recording, and child span creation.

3. **Public `ErrorBoundary` for server actions** — `spiceflow/react` now exports an `ErrorBoundary` component with `ErrorBoundary.ErrorMessage` and `ErrorBoundary.ResetButton` sub-components:
...
4. **Type-safe `json()` helper with phantom brands** — `import { json } from 'spiceflow'` wraps `Response.json()` with typed response phantom types so the fetch client gets full type safety for each status code:
...
5. **`.loader()` route kind for server-side data loading** — loaders run before page and layout handlers, with their return values merged by path specificity and passed to handlers via `ctx.loaderData`. Wildcard patterns like `/*` match all routes for global data:
   ...
   `getLoaderData()` provides non-React access to loader data outside components. Loader response status and headers are now forwarded to flight and HTML responses. Loaders only run when the request matches a React page or layout.

6. **`parseFormData` for type-safe form validation** — validates FormData against any Standard Schema with automatic string→number/boolean coercion:
...
7. **Path-scoped middleware** — `.use(path, handler)` limits middleware to a specific path or wildcard prefix:
...
8. **Every server action call refreshes the page automatically** — form submissions, direct imported action calls, and client wrappers all apply the new RSC payload immediately after the action finishes. `router.refresh()` is fire-and-forget. `throw redirect()` in server actions now works correctly (encoded in the RSC flight payload instead of raw HTTP 307).

9. **`router` direct import from `spiceflow/react`** — client-side navigation singleton with `push`, `replace`, `back`, `forward`, `refresh`, `href`, `pathname`, and `subscribe`. Works with `useRouterState()` hook and `router.searchParams` getter:
   ...
   `router.push()` now follows `.get()` 302 redirects via SPA navigation, preserving layout state and client component state. `getRouter()` is deprecated in favor of the direct import.

10. **`.staticGet()` — pre-render API routes at build time** — handlers run once during `vite build` and the response body is written to `dist/client/` as a static file:
...
11. **Self-contained Node.js build output + Vercel deployment** — `dist/` includes all runtime dependencies traced by `@vercel/nft`. Copy `dist/` into a Docker container with no install step. When `VERCEL=1` is set during build, spiceflow automatically generates `.vercel/output/` with static assets on CDN and a single serverless function.

12. **Auto-coerce query parameters to schema types** — `z.number()`, `z.boolean()`, and `z.array(z.string())` work directly without `z.coerce.number()` or `z.preprocess()`:
...
13. **Typed `Head` sub-components** — `Head.Meta`, `Head.Title`, `Head.Link`, `Head.Script`, `Head.Style`, and `Head.Base` each provide IDE autocomplete for their attributes. `document.title` updates on client-side navigation. Layout-based 404 handling — layouts receive `children` as `null` when no page matches.

14. **Object-style `.page()` with query schema support** — pass options as an object and declare typed query params:
...
15. **HTTP response headers for page, layout, and API handlers** — set response headers via `props.response.headers` in any handler. Response status is now consistent across all route types via `context.response`.

16. **Auto-configure `optimizeDeps.entries` per Vite environment** — no more manual `optimizeDeps.include` lists. `vite build` now builds all environments without `--app`.

17. **Cloudflare Workers support** — `app.handle()` owns the Flight→HTML bridge. Durable Objects and other named exports are preserved in builds. `waitUntil` auto-detection via `cloudflare:workers`. Fixed `require is not defined` error by explicitly including `@vitejs/plugin-rsc` vendored CJS files in `commonjsOptions.include`.

18. **`waitUntil` auto-detection for Vercel** — Spiceflow detects Vercel's request context for `waitUntil`. `preventProcessExitIfBusy` middleware now tracks pending `waitUntil` promises before exiting.

19. **Generic federation APIs** — `encodeFederationPayload()`, `RenderFederatedPayload`, and `decodeFederationPayload()` for routes returning plain objects, JSX, or async iterables. Same-process federation via `app.handle(new Request(...))`. Federation response format changed from JSON to SSE. `isolateStyles` prop on `RemoteComponent` for Shadow DOM style isolation. `signal` prop for cancellation on disconnect. Import map generation for dev-mode external URL components.

20. **`.server.ts` file boundary guard** — clear error when `*.server.ts` files are imported from client components during development.

21. **Improved React SSR performance** — GET/HEAD requests skip extra Flight decode pass. Bootstrap script content is cached in production. Improved Node.js adapter with backpressure handling and `pipeline()` streaming.

22. **Fixed Vite 8 dev-server crash under pnpm scoped packages** — nested-id syntax for `optimizeDeps.include`. Fixed duplicate React context crash by excluding `spiceflow` from `optimizeDeps`. Fixed `@tailwindcss/vite` triggering full page reloads during RSC HMR.

23. **Actionable errors for client-only React APIs in Server Components** — `TypeError: useState is not a function` now reads `useState only works in Client Components. Add the "use client" directive`.

24. **React 404 page with correct HTTP 404 status** for unmatched browser routes, wrapped in layouts. Fixed API routes shadowed by layout-only React matches.

25. **Fixed hydration crash on initial page load** — thenable handling in `createFromReadableStream`. Fixed `useId()` hydration mismatch in production builds. Fixed locked request bodies inside server actions.

26. **`publicDir` and `distDir` exports** — server components locate `public/` and `dist/` directories without guessing from `import.meta.dirname`.

27. **`ProgressBar.start()` and `ProgressBar.end()`** for driving the top loading bar around manual async work.

28. **Hide pages from OpenAPI output** — only API endpoints and `staticGet` routes appear. New [OpenAPI guide](./docs/openapi.md) covers advanced patterns.

29. **Removed deployment skew cookie mechanism** — the `spiceflow-deployment` cookie and 409 mismatch responses are gone. `getDeploymentId()` is still exported for analytics and cache keys.

30. **Improved prerender build errors** — static page build failures now show the full stack trace. `staticPage()` writes `.html` files to disk at build time.

31. **Changelog fixes**:
    - Fixed `React.cache()` deduplication in server components via `resolve.dedupe`
    - Fixed `<Head.Title>` layout override bug after hydration
    - Fixed `<Head.Link>` deduplication with `media` attribute
    - Fixed federation stream cancellation
    - Fixed `serveStatic()` EISDIR error on Node
    - Fixed route priority for child apps over parent wildcards
    - Fixed `app.listen()` running in Vite dev (now a noop with `import.meta.hot`)
    - Fixed `throw redirect()` inside page handlers returning correct 307
    - Fixed route matching specificity and wildcard param extraction
    - Fixed multi-environment Vite dep optimization for SSR/RSC cold starts
    - Fixed duplicate spiceflow instances under pnpm strict isolation
    - Fixed stale `router.refresh()` data with `Cache-Control: no-store`
    - Fixed `onError` firing multiple times per server action error
    - Default export for `spiceflow/vite` (named export `spiceflowPlugin` deprecated)
    - RSC/SSR debug logs gated behind `SPICEFLOW_VERBOSE=1`
    - `zod` moved to peer dependency; `react-server-dom-webpack` dependency removed
