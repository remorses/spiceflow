...
export default function AbortTestPage() {
  ...
  const [taskController, setTaskController] = useState<AbortController | null>(null);
  ...
  const startLongRunningTask = async () => {
    ...
    try {
      ...
      if ((error as Error).message.includes('aborted')) {
        setTaskStatus('Task aborted: ' + (error as Error).message);
      } else {
        setTaskStatus('Task failed: ' + (error as Error).message);
      }
    } finally {
  ...
  const startStream = async () => {
    ...
    try {
      ...
      if ((error as Error).message.includes('aborted')) {
        setStreamStatus('Stream aborted: ' + (error as Error).message);
      } else {
        setStreamStatus('Stream failed: ' + (error as Error).message);
      }
    } finally {
